...
curl -XPUT ${functionUrl}fs/upload?path=/entrypoint_hook.sh \
     ...
     --data-binary @src/pwnctl.worker/data/entrypoint_hook.sh

curl -XPUT ${functionUrl}fs/upload?path=/seed/task-definitions.yml \
     -H "X-Api-Key: $apiKey" \
     -H 'Content-Type: text/plain' \
     --data-binary @src/pwnctl.worker/data/seed/task-definitions.yml

curl -XPUT ${functionUrl}fs/upload?path=/seed/notification-rules.yml \
     -H "X-Api-Key: $apiKey" \
     -H 'Content-Type: text/plain' \
     --data-binary @src/pwnctl.worker/data/seed/notification-rules.yml
